...
class ConstructorResolver {
	...
	/**
	 * “自动装配构造函数”(根据类型使用构造函数参数)行为。
	 * 如果指定了显式构造函数参数值，将所有剩余参数与来自bean工厂的bean匹配，也将应用。
	 * <p>这对应于构造函数注入:在这种模式下，Spring bean工厂能够承载期望基于构造函数的依赖解析的组件。
	 *
		...
		if (constructorToUse == null) {
			...
			for (Constructor<?> candidate : candidates) {
				...
				if (constructorToUse != null && argsToUse.length > paramTypes.length) {
					// 已经找到可以满足的贪婪构造函数 -> 不要再往下看了，只剩下不那么贪婪的构造函数了。
					break;
				}
