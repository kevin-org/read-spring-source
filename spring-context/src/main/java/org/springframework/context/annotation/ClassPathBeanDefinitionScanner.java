...
public class ClassPathBeanDefinitionScanner extends ClassPathScanningCandidateComponentProvider {
	...
	public int scan(String... basePackages) {
		...
		System.out.println("==========spring 注册流程 扫描 basePackages==========");
		doScan(basePackages);
	...
	protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		...
		for (String basePackage : basePackages) {
			System.out.println("==========spring 注册流程 扫描 basePackages -- 获取所有组件，并执行基础初始化==========");
	...
	/**
	 * 在扫描组件类检索到的内容之外，对给定的bean定义应用进一步的设置。
	...
	protected void registerBeanDefinition(BeanDefinitionHolder definitionHolder, BeanDefinitionRegistry registry) {
		System.out.println("==========spring 注册流程 扫描 basePackages -- 注册Bean定义==========");
		BeanDefinitionReaderUtils.registerBeanDefinition(definitionHolder, registry);
	}


	/**
	 * 检查给定的候选bean名称，确定相应的bean定义是否需要注册或与现有定义冲突
	 *
	 ...
	 *
	 * <p>当现有的bean定义来自同一源或来自非扫描源时，默认实现认为它们是兼容的。
	 *
	...
	protected boolean isCompatible(BeanDefinition newDefinition, BeanDefinition existingDefinition) {
		return (!(existingDefinition instanceof ScannedGenericBeanDefinition) ||  //显式注册的重写bean
				newDefinition.getSource().equals(existingDefinition.getSource()) ||  //扫描同一个文件两次
