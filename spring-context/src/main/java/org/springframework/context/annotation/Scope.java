...
public @interface Scope {
	...
	/**
	 * 指定要用于注释组件/bean的作用域的名称。<p>默认为空字符串({@code ""})，表示
	 ...
	 * <p>默认为{@link ScopedProxyMode#DEFAULT}，这通常表示不应该创建作用域代理，除非在组件扫描指令级别配置了不同的默认值。
	 *
	 * <p>类似于Spring XML中的{@code <aop:scope -proxy/>}支持。
	 * @see ScopedProxyMode
	 */
